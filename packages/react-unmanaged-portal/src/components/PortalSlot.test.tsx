...
describe('PortalSlot', () => {
  ...
  it('기본 포털 ID로 렌더링', () => {
    const { container } = render(<PortalSlot slotKey="main" />)
  ...
  it('커스텀 포털 ID로 렌더링', () => {
    const { container } = render(
      <PortalSlot portalId="custom-portal" slotKey="main" />,
    )
  ...
  it('커스텀 컨테이너 태그로 렌더링', () => {
    const { container } = render(<PortalSlot slotKey="main" as="section" />)
  ...
  it('슬롯 키가 변경되면 타겟이 업데이트됨', () => {
    const { container, rerender } = render(<PortalSlot slotKey="slotKey1" />)
    ...
    rerender(<PortalSlot slotKey="slotKey2" />)
  ...
  it('언마운트 시 타겟이 제거됨', () => {
    const { container, unmount } = render(<PortalSlot slotKey="main" />)
  ...
  it('여러 PortalSlot이 다른 슬롯 키로 등록됨', () => {
    const { container: container1 } = render(<PortalSlot slotKey="slotKey1" />)
    const { container: container2 } = render(<PortalSlot slotKey="slotKey2" />)
  ...
  it('같은 슬롯 키로 여러 PortalSlot을 렌더링하면 마지막 것이 등록됨', () => {
    const { unmount: unmount1 } = render(
      <PortalSlot slotKey="main" id="slot1" />,
    )
    const { container: container2 } = render(
      <PortalSlot slotKey="main" id="slot2" />,
    )
  ...
  it('다른 포털 ID로 여러 PortalSlot을 독립적으로 관리', () => {
    const { container: container1 } = render(
      <PortalSlot portalId="portal1" slotKey="main" />,
    )
    const { container: container2 } = render(
      <PortalSlot portalId="portal2" slotKey="main" />,
    )
